(function () {
  ...
  window.ShowNotes = function (NotesJson) {
    try {
      _Notes = JSON.parse(NotesJson) || [];
    } catch (e) {
      _Notes = [];
    }

    var visible = _Notes.filter(function (a) {
      return !_dismissed[a.entryNo];
    });
  ...
  function reconcile(visible) {
    ...
    visible.forEach(function (Note) {
      ...
      if (_rendered[Note.entryNo] && _rendered[Note.entryNo].isConnected)
        return;
    ...
    function onMouseMove(e) {
      ...
      var newLeft = Math.max(
        ...
        Math.min(
          ...
          win.innerWidth - noteEl.offsetWidth,
        ),
      );
      var newTop = Math.max(
        0,
        Math.min(
          ...
          win.innerHeight - noteEl.offsetHeight,
        ),
      );
  ...
  function watchForRemoval() {
    ...
    try {
      ...
      _removalObserver.observe(_parentDoc.body, {
        childList: true,
        subtree: true,
      });
    } catch (e) {
      /* cross-origin or not supported */
    }
  }
